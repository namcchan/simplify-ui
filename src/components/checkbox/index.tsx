import { Checkbox as MuiCheckbox, CheckboxProps, SvgIcon, useTheme } from '@mui/material';

type Props = Omit<CheckboxProps, 'color'> & {
  color?: 'primary' | 'secondary' | 'success' | 'error' | 'info' | 'warning';
};

const Checkbox = ({ color = 'primary', ...props }: Props) => {
  const { palette } = useTheme();

  const iconColor = color ? palette[color].main : palette.primary.main;

  return (
    <MuiCheckbox
      {...props}
      indeterminateIcon={
        <SvgIcon>
          <svg fill={iconColor} focusable="false" aria-hidden="true" viewBox="0 0 24 24">
            <path d="M17,2 C19.7614,2 22,4.23858 22,7 L22,7 L22,17 C22,19.7614 19.7614,22 17,22 L17,22 L7,22 C4.23858,22 2,19.7614 2,17 L2,17 L2,7 C2,4.23858 4.23858,2 7,2 L7,2 Z M15,11 L9,11 C8.44772,11 8,11.4477 8,12 C8,12.5523 8.44772,13 9,13 L15,13 C15.5523,13 16,12.5523 16,12 C16,11.4477 15.5523,11 15,11 Z"></path>
          </svg>
        </SvgIcon>
      }
      checkedIcon={
        <SvgIcon>
          <svg aria-hidden="true" viewBox="0 0 24 24">
            <path
              fill={iconColor}
              d="M17 2a5 5 0 0 1 5 5v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a5 5 0 0 1 5-5Zm-1.625 7.255-4.13 4.13-1.75-1.75a.881.881 0 0 0-1.24 0c-.34.34-.34.89 0 1.24l2.38 2.37c.17.17.39.25.61.25.23 0 .45-.08.62-.25l4.75-4.75c.34-.34.34-.89 0-1.24a.881.881 0 0 0-1.24 0Z"
            ></path>
          </svg>
        </SvgIcon>
      }
      icon={
        <SvgIcon>
          <svg aria-hidden="true" viewBox="0 0 24 24">
            <path d="M17.9 2.318A5 5 0 0 1 22.895 7.1l.005.217v10a5 5 0 0 1-4.783 4.995l-.217.005h-10a5 5 0 0 1-4.995-4.783l-.005-.217v-10a5 5 0 0 1 4.783-4.996l.217-.004h10Zm-.5 1.5h-9a4 4 0 0 0-4 4v9a4 4 0 0 0 4 4h9a4 4 0 0 0 4-4v-9a4 4 0 0 0-4-4Z"></path>
          </svg>
        </SvgIcon>
      }
    />
  );
};

export default Checkbox;
